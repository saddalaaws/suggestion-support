...
module "deploy_provider_setup" {
  ...
  deploy_account_name     = local.account_name
...
provider "aws" {
  region  = local.cdp_settings_from_ics.region
...
provider "aws" {
  # FIXME: Remove after next prod deployment
  alias   = "main"
...
provider "aws" {
  alias   = "backup"
  region  = local.cdp_settings_from_ics.backup_region
  profile = module.deploy_provider_setup.deploy_settings.deploy_profile
  default_tags { tags = merge(module.deploy_provider_setup.deploy_default_tags, { TerraformProvider = "backup" }) }
  dynamic "assume_role" {
    for_each = module.deploy_provider_setup.deploy_settings.role_arn == null ? [] : [0]
    content {
      session_name = module.deploy_provider_setup.session_name
